...
window.AddressData = (function() {
  ...
  async function loadAddressData() {
    ...
    try {
      // Load both address files in parallel
  ...
  function populateSelectOptions(selectElement, options, placeholder = "Chọn...") {
    ...
    if (options.length > 0) {
      optionsContainer.innerHTML = options
        .map(option => 
          `<div class="dropdown-option" data-value="${option.code}" data-codename="${option.codename || ''}">
            ${option.name}
          </div>`
        )
        .join('');
  ...
  function setupNormalAddressCascading() {
    ...
    provinceSelect.addEventListener('change', function(e) {
      if (e.detail && e.detail.value) {
        ...
        provinceSelect.dispatchEvent(new CustomEvent('locationChange', {
    ...
    districtSelect.addEventListener('change', function(e) {
      if (e.detail && e.detail.value) {
        ...
        const selectedProvince = provinceSelect.querySelector('.dropdown-option.selected');
        if (selectedProvince) {
          const provinceCode = parseInt(selectedProvince.getAttribute('data-value'));
          ...
          // Trigger custom event
          districtSelect.dispatchEvent(new CustomEvent('locationChange', {
            detail: { value: e.detail.value, text: e.detail.text }
          }));
        }
      }
  ...
  function setupNewAddressCascading() {
    ...
    newProvinceSelect.addEventListener('change', function(e) {
      if (e.detail && e.detail.value) {
        ...
        // For new system, directly load all wards from the first district
        const districts = getDistricts34(provinceCode);
        if (districts.length > 0) {
          const wards = getWards34(provinceCode, districts[0].code);
          populateSelectOptions(newWardSelect, wards, "Xã/Phường");
        } else {
          resetSelect(newWardSelect, "Xã/Phường");
        }
        
        // Trigger custom event
        newProvinceSelect.dispatchEvent(new CustomEvent('locationChange', {
          detail: { value: e.detail.value, text: e.detail.text }
        }));
      }
    });
  ...
  function getWardByCode(provinceCode, districtCode, wardCode, useNewSystem = false) {
    const district = getDistrictByCode(provinceCode, districtCode, useNewSystem);
