 ...
 * Quản lý thông tin NGƯỜI GỬI (shop): chọn người gửi, COD, thanh toán, bưu cục
 */
...
window.Receiver = (function () {
  ...
  async function loadReceiverList() {
    ...
    activeReceivers.forEach((receiver) => {
      ...
      option.textContent = `${receiver.name || ""} - ${address || ""} - ${
        receiver.phone || ""
      }`;
  ...
  function initReceiverSelectChange() {
    ...
    receiverSelect.addEventListener("change", function () {
      ...
      const receiver = currentReceivers.find(
        (r) => String(r._id || r.id) === selectedId
      );
      if (!receiver) return;
  ...
  // ===== Post office selector (khôi phục đúng logic cũ) =====
  ...
  function initPaymentNotification() {
    ...
    const receiverPaymentModal = document.getElementById(
      "receiverPaymentModal"
    );
    const confirmSenderPaymentBtn = document.getElementById(
